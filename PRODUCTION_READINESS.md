...
| **D10** | P2 | L1 / platform | `IMPLEMENTED / AWAITING RE-REVIEW` | `app.stop()` — единый idempotent coordinator: server close, owned CLI/native process и auth Chrome cleanup выполняются конкурентно в пределах 10 s; success требует confirmed target termination. Windows exact-PID tree kill, Unix runner/launcher confirmation и macOS exact window helper bounded 5 s / 2 s; unconfirmed ownership сохраняется и даёт typed `SHUTDOWN_INCOMPLETE`. | `TEST`: первый independent review дал FAIL по двум races; follow-up добавил bounded native PID capture с retained ownership и отделил AUTH logical abort от authoritative Chrome tree kill. 34 net-new regressions / 36 files / 938 tests; focused lifecycle suites 70/70. Re-review и desktop live PB39 ещё не выполнены. | D8 telemetry | — |
...
| G2 | `npm test` 100% green | PASS: 938/938 on D10 follow-up branch (recheck release commit) |
