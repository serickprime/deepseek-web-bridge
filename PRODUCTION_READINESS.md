...
| **D10** | P2 | L1 / platform | `IMPLEMENTED / AWAITING INDEPENDENT REVIEW` | `app.stop()` — единый idempotent coordinator: server close, owned CLI/native process и auth Chrome cleanup выполняются конкурентно в пределах 10 s; success требует confirmed target termination. Windows exact-PID tree kill, Unix runner/launcher confirmation и macOS exact window helper bounded 5 s / 2 s; unconfirmed ownership сохраняется и даёт typed `SHUTDOWN_INCOMPLETE`. | `TEST`: 29 net-new regressions / 36 files / 933 tests; taskkill success/non-zero/spawn-error/hang/already-exited/target-alive, retained ownership, tracked/untracked, auth Chrome, Linux SIGTERM/failure/timeout, macOS closed/not-found/non-zero/error/hang/exact target, route acknowledgement/auth preservation, signal path и coordinator idempotence/deadline. Focused lifecycle suites 65/65. Independent review и desktop live PB39 ещё не выполнены. | D8 telemetry | — |
...
| G2 | `npm test` 100% green | PASS: 933/933 on D10 implementation branch (recheck release commit) |
...
| G15 | Shutdown не оставляет orphan/stale PID и не убивает чужие процессы | NEEDS INDEPENDENT REVIEW + DESKTOP LIVE: D10 implementation deterministic PASS |
...
остаются открытыми. D10 — `IMPLEMENTED / AWAITING INDEPENDENT REVIEW`, поэтому
